...
@section('content')
                        ...
                        @foreach ($reservasi as $item)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="modal fade" id="notaNotAvailableModal-{{ $item->id }}" tabindex="-1"
        ...
        <div class="container py-5">
            <div class="mx-auto text-center wow fadeIn" data-wow-delay="0.1s" style="max-width: 600px;">
                @if ($member->isEmpty())
                    ...
                    <a href="{{ route('member.create') }}" class="btn btn-light">Daftar Sekarang</a>
                @else
                    ...
                    @foreach ($member as $m)
                        <h5 class="text-white">{{ $m->tgl_mulai }} <span class="fw-light text-dark">s/d</span> {{ $m->tgl_akhir }}</h5>
                    @endforeach
                @endif
            </div>
        </div>
    ...
    <script>
        function checkStatusAndPrintReceipt(status) {
            if (status === 'Sukses') {
                window.location.href = "/transaksi/{{ $reservasi->first()->id }}/nota";
            } else {
                $('#notaNotAvailableModal-{{ $item->id }}').modal('show');
            }
        }
    </script>
@endsection
